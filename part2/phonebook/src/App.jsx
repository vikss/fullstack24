...
const App = () => {
  ...
  const handleForm = (event) => {
    ...
    if (!namePresent) {
      console.log('Creating a new phonebook entry for this person ', personObj)
      personService
        .create(personObj)
        .then((res) => {
      ...
      if (
        ...
        let oldPersonEntry = persons.find(
        ...
        personService.updateNumber(oldPersonEntry.id, personObj).then((res) => {
          let newPersonsArr = persons.filter(
            (person) => person.id != oldPersonEntry.id
          )
    ...
    if (window.confirm(`Delete ${personName}?`)) {
      personService
        ...
        .then((res) => {
          ...
          console.log(
            'Persons array after delete operation is ',
            newPersonsArray
          )
...
const Notification = ({ message, error }) => {
  ...
  else if (message) return <div className='message'>{message}</div>
  return <div></div>
}
const Filter = (props) => {
  return (
